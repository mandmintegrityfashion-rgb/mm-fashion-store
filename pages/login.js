      ...
      <div className="min-h-screen bg-[#F7FAFC] flex items-center justify-center px-4 py-12">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: [0.4, 0, 0.2, 1] }} className="w-full max-w-md">
          <div className="text-center mb-8">
            <h1 className={`${playfair.className} text-3xl font-bold text-[#1F2D3D] mb-2`}>Welcome Back</h1>
          ...
          <div className="bg-white rounded-2xl border border-[#E6F0FA] p-8" style={{ boxShadow: "0 4px 16px rgba(31,45,61,0.06)" }}>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <div className={`flex items-center border rounded-xl px-4 py-3 transition-all duration-200 ${focused.email ? "border-[#4C9EFF] shadow-[0_0_0_3px_rgba(76,158,255,0.08)]" : "border-[#D4E3F7]"}`}>
              ...
              <div>
                ...
                <div className={`flex items-center border rounded-xl px-4 py-3 transition-all duration-200 ${focused.password ? "border-[#4C9EFF] shadow-[0_0_0_3px_rgba(76,158,255,0.08)]" : "border-[#D4E3F7]"}`}>
                  ...
                  <input type={showPassword ? "text" : "password"} name="password" placeholder="Enter your password" value={form.password} onChange={handleChange}
                    ...
                    className="w-full bg-transparent outline-none text-sm text-[#1F2D3D] placeholder:text-[#B8BCC6]" autoComplete="current-password" />
            ...
            <p className="text-sm text-[#5A6171] text-center">
              ...
              <Link href={`/register${redirect ? `?redirect=${encodeURIComponent(redirect)}` : ""}`} className="text-[#1A5DAB] font-semibold hover:text-[#4C9EFF] transition-colors">
                Create Account
              </Link>
            </p>
          </div>
