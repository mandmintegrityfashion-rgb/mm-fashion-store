    ...
    <main className="w-full lg:w-full flex flex-col gap-12 px-4 lg:px-0">
            ...
            {editingAddress ? (
              <div className="flex flex-col gap-3">
                ...
                <div className="flex gap-2">
                  <button onClick={saveAddress} className="px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700">Save</button>
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setEditingAddress(true)} className="text-yellow-600 text-sm hover:underline">Edit</button>
              </div>
            )}
          </div>
        </div>
      ...
      <section className="bg-[#E8F2E6] py-12 rounded-xl shadow-inner">
        <div className="flex items-center justify-between mb-8 px-6">
          ...
          <button className="text-yellow-600 hover:underline text-sm">See All →</button>
        </div>
          ...
          return <>
            ...
            {moreVisible < validProducts.length && (
              <div className="text-center mt-8">
                <button onClick={() => setMoreVisible((prev) => Math.min(prev + increment, validProducts.length))} className="px-6 py-2 rounded-full bg-[#546258] text-white font-semibold hover:bg-[#3f4849] transition">View More</button>
              </div>
            )}
          </>;
        })()}
