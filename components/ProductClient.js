...
export default function ProductClient({ product }) {
  ...
  return (
    <>
      ...
      <div className={playfair.variable + " " + overpass.variable + " min-h-screen py-8 px-4 sm:px-8 bg-[#F7FAFC]"}>
        <div className="max-w-7xl mx-auto">
          <button onClick={() => router.back()} className="mb-6 flex items-center gap-2 text-[#5A6171] hover:text-[#1F2D3D] text-sm font-medium transition-colors">
            <FiArrowLeft size={16} /> Back
          </button>

          <div className="grid md:grid-cols-[3fr_2fr] gap-10 items-start">
            {/* Images */}
            <div className="bg-white rounded-2xl p-5 border border-[#E6F0FA]" style={{ boxShadow: "0 2px 12px rgba(31,45,61,0.04)" }}>
                ...
                {productImages.map((img, idx) => (
                  <button key={idx} onClick={() => setActiveImage(img)}
                    className={"w-16 h-16 rounded-lg border-2 overflow-hidden transition-all duration-200 flex-shrink-0 " +
                      (img === activeImage ? "border-[#4C9EFF]" : "border-[#D4E3F7] hover:border-[#4C9EFF]/50")}>
            ...
            {/* Details */}
            <div className="bg-white rounded-2xl p-7 border border-[#E6F0FA] flex flex-col gap-5" style={{ boxShadow: "0 2px 12px rgba(31,45,61,0.04)" }}>
              <h1 className={playfair.className + " text-3xl md:text-4xl font-bold text-[#1F2D3D] tracking-tight"}>{product.name}</h1>

              <div className="flex items-center gap-3">
                {product.promoPrice && product.promoPrice < product.salePriceIncTax ? (
                  <>
                    <span className="text-2xl font-bold text-[#4C9EFF]">{"\u20A6"}{product.promoPrice.toLocaleString()}</span>
                  ...
                  <span className="text-2xl font-bold text-[#1F2D3D]">{"\u20A6"}{product.salePriceIncTax?.toLocaleString()}</span>
                )}
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex items-center border border-[#D4E3F7] rounded-xl overflow-hidden">
                  <button onClick={() => setQuantity((p) => Math.max(1, p - 1))}
                    className="w-10 h-10 flex items-center justify-center hover:bg-[#F1F5F9] text-[#5A6171] transition-colors">
                    <FiMinus size={14} />
                  </button>
                  <span className="w-10 h-10 flex items-center justify-center text-sm font-semibold text-[#1F2D3D] border-x border-[#D4E3F7]">{quantity}</span>
              ...
              <motion.button onClick={handleAddToCart} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}
                className={"flex items-center gap-3 justify-center px-6 py-4 rounded-xl font-semibold text-sm transition-all " +
                  (addedToCart ? "bg-green-600 text-white" : "bg-[#1F2D3D] text-white hover:bg-[#1A2332]")}>
          ...
          {/* Reviews */}
          <div className="bg-white max-w-7xl mx-auto mt-12 rounded-2xl border border-[#E6F0FA] p-8" style={{ boxShadow: "0 2px 12px rgba(31,45,61,0.04)" }}>
            <h2 className={playfair.className + " text-2xl font-bold mb-8 text-[#1F2D3D] border-b border-[#E6F0FA] pb-4"}>Customer Reviews</h2>

            <div className="flex flex-col md:flex-row gap-8">
              <div className="md:w-1/2 bg-[#F1F5F9] p-6 rounded-xl">
                <ReviewForm productId={product._id} onReviewAdded={handleReviewAdded} />
              </div>

              <div className="md:w-1/2 space-y-4">
                <h3 className="text-lg font-semibold text-[#1F2D3D] border-b border-[#E6F0FA] pb-3 mb-4">All Reviews</h3>
                {reviews.length > 0 ? (
                  reviews.map((r, i) => (
                    <div key={r._id || i} className="bg-[#F7FAFC] border border-[#E6F0FA] rounded-xl p-5 hover:border-[#D4E3F7] transition-colors">
                      <div className="flex justify-between items-center mb-2">
                        <p className="font-semibold text-[#1F2D3D] text-sm">{r.customer?.name || r.customerName || "Anonymous"}</p>
                        <div className="flex items-center gap-0.5">
                          {[...Array(5)].map((_, k) => (
                            <svg key={k} className={"w-4 h-4 " + (k < r.rating ? "text-[#4C9EFF]" : "text-[#D4E3F7]")} fill="currentColor" viewBox="0 0 20 20">
                      ...
                      {r.title && <h4 className="font-semibold text-sm text-[#1F2D3D] mb-1">{r.title}</h4>}
